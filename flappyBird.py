...
def gameStart(bird):
    bird.__init__()
    pygame.mixer.pause()
    font = pygame.font.Font('04B_19.ttf', 150)
    headingSuface = font.render('FLAPPY BIRD', True, (255, 0, 0))
    headingSize = headingSuface.get_size()
    
    font = pygame.font.Font('04B_19.ttf', 80)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                music = pygame.mixer.Sound('flappybird_song.wav')
                music.play(loops = -1)
                return
...
def gameOver(bird, columns, score):
    pygame.mixer.pause()
    music_end = pygame.mixer.Sound('flappy_die.mp3')
    music_end.play()
    font = pygame.font.Font('04B_19.ttf', 140)
    ...
    font = pygame.font.Font('04B_19.ttf', 50)
    closecamSuface = font.render('Press "q" to close the cam window', True, (0, 0, 0))
    closecamSize = commentSuface.get_size()
    ...
    while True:
        ...
        DISPLAYSURF.blit(headingSuface, (int((WINDOWWIDTH - headingSize[0])/2), 100))
        DISPLAYSURF.blit(commentSuface, (int((WINDOWWIDTH - commentSize[0])/2), 500))
        DISPLAYSURF.blit(scoreSuface, (int((WINDOWWIDTH - scoreSize[0])/2), 250))
        DISPLAYSURF.blit(closecamSuface, (int((WINDOWWIDTH - closecamSize[0])/6), 600))
        pygame.display.update()
        fpsClock.tick(FPS)
